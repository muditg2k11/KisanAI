  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 201) {
        // Auto-login: save user data and go straight to dashboard
      ...
      if (err.response) {
        switch (err.response.status) {
          case 400: setError('Invalid form data.'); break;
          case 500: setError('Server error. Please try later.'); break;
          default: setError('Registration failed.');
        }
      } else {
    ...
    <>
      ...
      <div className="cgpt-auth-page">
        <div className="cgpt-auth-box">
          ...
          <h1 className="cgpt-title">Farmer Registration</h1>

          {error && <div className="cgpt-error"><i className="fa fa-exclamation-circle"></i> {error}</div>}
          {success && <div className="cgpt-success"><i className="fa fa-check-circle"></i> {success}</div>}
